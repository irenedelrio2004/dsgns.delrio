...
(() => {
  ...
  const stylesheet =
    ...
    '.frame{position:absolute;inset:0;overflow:hidden;background:var(--slot-bg,rgba(127,127,127,.08))}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      const hasAuthorImage = !!srcAttr;
      this._userUrl = hasAuthorImage ? null : (stored && stored.u) || null;
      const url = srcAttr || this._userUrl;
      const zoomAttr = Number.parseFloat(this.getAttribute('zoom') || '1');
      const initialZoom = Number.isFinite(zoomAttr) ? clampS(zoomAttr) : 1;
      const panYAttr = Number.parseFloat(this.getAttribute('pan-y') || '0');
      const initialPanY = Number.isFinite(panYAttr) ? panYAttr : 0;
      // Don't clobber an in-flight reframe with a store-triggered re-render.
      if (!this.hasAttribute('data-reframe')) {
        this._view = {
          s: !hasAuthorImage && stored && Number.isFinite(stored.s) ? clampS(stored.s) : initialZoom,
          x: !hasAuthorImage && stored && Number.isFinite(stored.x) ? stored.x : 0,
          y: !hasAuthorImage && stored && Number.isFinite(stored.y) ? stored.y : initialPanY,
        };
      }
